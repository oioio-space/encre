...
#
# Set BENCH_PATTERN to scope to a subset (e.g. BENCH_PATTERN=BenchmarkParse); the
# default "." runs every benchmark in the module.
set -euo pipefail
...
bench() { go test -run '^$' -bench="${BENCH_PATTERN:-.}" -benchmem -count="${BENCH_COUNT:-10}" ./...; }
