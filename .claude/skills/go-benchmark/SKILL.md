...
Benchmark a function when: it's on a hot path (called in tight loops — e.g. a parser inner loop
or a per-request scoring function), it allocates heavily, or someone claims it's "slow". Do NOT
...
4. **Algorithmic** improvement (better complexity) beats micro-tuning — escalate to
   `architect` if the win is algorithmic.
