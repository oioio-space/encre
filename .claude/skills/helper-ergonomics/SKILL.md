...
description: Use when adding or changing exported Go API (or the CLI) in this project, and before committing — this project is human-facing, so hunt for convenience/helper functions that make it easy to use. Spots missing one-call wrappers, constructors, broad-input helpers, and option patterns. A pre-commit hook reinforces it.
---

# Helper Ergonomics

Goal: **make the easy thing easy.** This project is meant to be used by humans — both as a Go
library and as a CLI — so a newly added or changed API must be judged not only for correctness
but for *how little a caller has to write to do the common thing*. Before committing API changes,
hunt for helper/convenience functions that collapse boilerplate, and propose the ones that are
earned.
...
Concrete (illustrative) example: if the naive path to do the common thing is
```go
import _ "github.com/oioio-space/encre/defaults" // easy to forget → zero results
in, _ := os.Open(path)
eng, _ := myproj.New(in, myproj.Config{})
prog, res := eng.Run(ctx)
best := (<-res).BestGuess
```
a human-friendly helper collapses that to one obvious call:
```go
best, err := myproj.RunFile(ctx, path)      // or Run(ctx, in, opts...)
```
...
7. **Actionable errors.** Error messages should tell the human what to do ("config file not
   found at %s; run 'myproj init' to create one"), not just what failed.
