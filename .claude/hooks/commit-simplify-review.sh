...
- REUSE: does new code re-implement something the repo already has? Grep the
  project's existing packages for a helper that already does this before writing
  a fresh copy.
...
- EFFICIENCY: wasted work the diff adds — redundant recompute or I/O, allocations
  ...
  the hot-path packages listed in CLAUDE.md, prove any perf-affecting change with
  benchstat — never by feel.
