...
if ! printf '%s' "$diff_added" | grep -qE '^\+[[:space:]]*(func|type|const|var)[[:space:]]+[A-Z]|cli\.|Flag\{|cmd/'; then
  # Also fire if a cmd/ file is staged (CLI ergonomics) even without the above.
  printf '%s' "$staged_go" | grep -q '^cmd/' || exit 0
fi

hook_emit_context "HELPER-ERGONOMICS PRE-COMMIT REVIEW (this project is human-facing):
